...
const EquipmentList = ({ equipmentList, setEquipmentList }) => {
  if (!equipmentList || equipmentList.length === 0) {
    return (
      <h1 className="text-2xl font-bold">No equipment details available.</h1>
    );
  }
  ...
  const filteredEquipment = equipmentList.filter((equipment) => {
    ...
    const matchType =
      typeFilter === "All Types" || equipment.type === typeFilter;
    const matchStatus =
      statusFilter === "All Status" || equipment.status === statusFilter;
