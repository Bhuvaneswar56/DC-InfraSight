...
const AlertDetailsPage = () => {
  ...
  const fetchAlertDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/infra/alerts/${id}`
      );
  ...
  const handleAcknowledge = async () => {
    try {
      await axios.put(`http://localhost:3000/api/infra/alerts/${id}/status`, {
        status: "acknowledged",
      });
      fetchAlertDetails();
    } catch (err) {
    ...
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        >
          ...
          <Button variant="outlined" onClick={fetchAlertDetails} sx={{ mt: 2 }}>
            Retry
          </Button>
        </Box>
      </Container>
